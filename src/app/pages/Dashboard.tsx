...
export function Dashboard() {
  ...
  const handleAIChat = () => {
    ...
    if (lowerInput.includes('meilleur') || lowerInput.includes('top')) {
      const topCandidates = mockCandidates
        .filter(c => c.status !== 'Rejected')
        .sort((a, b) => b.aiAnalysis.score - a.aiAnalysis.score)
        .slice(0, 3);
      aiResponse = `Top 3 candidates:\n${topCandidates.map(c => 
        `• ${c.name} (${c.aiAnalysis.score}/10) - ${c.aiAnalysis.profileType}`
      ).join('\n')}`;
    } else if (lowerInput.includes('ia') || lowerInput.includes('ai')) {
      const aiProfiles = mockCandidates.filter(c => 
        c.aiAnalysis.profileType.includes('AI') && c.status !== 'Rejected'
      );
      aiResponse = `Found ${aiProfiles.length} AI profiles:\n${aiProfiles.map(c => 
        `• ${c.name} (score: ${c.aiAnalysis.score}/10) - ${c.status}`
      ).join('\n')}`;
    } else if (lowerInput.includes('projet') || lowerInput.includes('plus de')) {
      const candidates = mockCandidates.filter(c => c.projects.length >= 2);
      aiResponse = `${candidates.length} candidates with 2+ projects:\n${candidates.map(c => 
        `• ${c.name} (${c.projects.length} projects) - ${c.aiAnalysis.profileType}`
      ).join('\n')}`;
    } else if (lowerInput.includes('insat')) {
      const insatCandidates = mockCandidates.filter(c => c.school === 'INSAT');
      aiResponse = `${insatCandidates.length} INSAT candidates:\n${insatCandidates.map(c => 
        `• ${c.name} (${c.aiAnalysis.score}/10) - ${c.status}`
      ).join('\n')}`;
    } else {
      aiResponse = `Currently tracking ${mockCandidates.length} applications with an average score of ${stats.avgScore}/10. ${stats.shortlisted} candidates are shortlisted and ${stats.accepted} have been accepted.`;
    }
              ...
              {chatMessages.length === 0 ? (
                <div className="text-center py-8 text-gray-500 dark:text-gray-400">
                  ...
                  <ul className="text-sm space-y-2 text-left max-w-md mx-auto">
                    <li className="p-2 bg-gray-50 dark:bg-gray-700 rounded">Show me the top AI profiles</li>
                    <li className="p-2 bg-gray-50 dark:bg-gray-700 rounded">Who has more than 2 projects?</li>
                    <li className="p-2 bg-gray-50 dark:bg-gray-700 rounded">INSAT candidates</li>
                  </ul>
                </div>
              ) : (
            ...
            <div className="p-5 border-t border-gray-200 dark:border-gray-700">
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-500 dark:placeholder-gray-400 focus:ring-2 focus:ring-indigo-500 dark:focus:ring-indigo-400 focus:border-transparent"
                />
                <button
                  onClick={handleAIChat}
                  className="px-6 py-2.5 bg-indigo-600 dark:bg-indigo-500 text-white rounded-lg hover:bg-indigo-700 dark:hover:bg-indigo-600 transition-colors font-medium shadow-sm"
                >
                  Send
                </button>
              </div>
